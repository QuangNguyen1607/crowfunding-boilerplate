...
const schema = yup.object().shape({
	...
	email: yup.string().email("Invalid email address").required('This field in required'),
	password: yup.string().min(8, 'Password must be 8 character').required('This field is required'),
	termVeri: yup.bool().oneOf([true], 'You must be check term is required').required('You must be check term is required')
})
const SignUpPage = () => {
	const {
		handleSubmit,
	...
	return (
		<>
			<LayoutAuthentication heading='Sign Up'>
				<p className='lg:text-sm text-xs text-text3 text-center mt-10px mb-6 lg:mb-8'>Dont have an account ?{" "}<Link to="/sign-in" className='text-primary cursor-pointer underline font-medium'>Sign In</Link></p>
				<button className='flex items-center w-full justify-center font-semibold text-base gap-x-2 border-strock-color py-3 border cursor-pointer select-none'>
					...
					<span className='text-text2 text-base'>Sign up with google</span>
				</button>
				<button className='text-center w-full py-3 text-text2 text-sm hover:underline'>Or sign up with email</button>
				<form onSubmit={handleSubmit(handleSignUp)}>
					<FormGroup>
						<Label htmlFor='name'>Full name *</Label>
						<Input control={control} name='name' type='text' placeholder='Lê Quang Nguyên' error={errors.name?.message}></Input>
					</FormGroup>
					<FormGroup>
						<Label htmlFor='email'>Email *</Label>
						<Input control={control} name='email' type='email' placeholder='lequangnguyen409@gmail.com' error={errors.email?.message}></Input>
					</FormGroup>
					<FormGroup>
						<Label htmlFor='password'>Password *</Label>
						<Input control={control} name='password' type='password' placeholder='Create a password' error={errors.password?.message}></Input>
					</FormGroup>
					...
					<div className="flex items-start gap-x-5 mb-5">
						<Checkbox control={control} checked={watchTerm} name='termVeri'></Checkbox>
						<p className='text-sm text-text2 flex-1 flex-shrink-0'>
							I agree to the <span className='text-secondary underline'>Terms of Use</span> and have read and
							understand the <span className='text-secondary underline'>Privacy policy</span>.
						</p>

					</div>

					<Button type='submit' className='bg-primary w-full'>Create my account</Button>
				</form>
